...
FROM
    customers AS r
        INNER JOIN
    addresses AS z ON r.customer_id = z.customer_id
WHERE
    (r.customer_id = z.customer_id)
        AND (r.shipping_address_id = z.address_id);
...
FROM
    customers a
        INNER JOIN
    orders b ON a.customer_id = b.customer_id
        INNER JOIN
    order_items x ON b.order_id = x.order_id
        INNER JOIN
    products y ON x.product_id = y.product_id
ORDER BY a.last_name , b.order_date , y.product_name;
